...
# from G3Gmdh.ECS.costcla.metrics import cost_loss
from sklearn.utils import column_or_1d

FitLayerData = namedtuple('FitLayerData',
                         ['sublayer', 'train_x', 'train_y',
                          'validate_x', 'validate_y','train_cost_mat',
                          'validate_cost_mat', 'params'])
...
class CriterionType(Enum):
    ...
    def get(arg):
        if isinstance(arg, CriterionType):
            ...
            return CriterionType.cmpCustom
        else:
            raise ValueError(arg)
...
class Neuron(object):
    ...
    def need_bias_stuff(self, criterion_type):
        if criterion_type == CriterionType.cmpCustom:
            return False
        return True
    ...
    def get_error(self, criterion_type):
        ...
        if criterion_type == CriterionType.cmpValidate:
            ...
            return self.bias_err
        elif criterion_type == CriterionType.cmpCustom:  # 新增
            return self.validate_err
        else:
    ...
    def cost_loss(self, y_true, y_pred, cost_mat):
        
        minority_samples = y_true == 1
        majority_samples = ~minority_samples
        minority_cost = 5*cost_mat[minority_samples]
        majority_cost = cost_mat[~minority_samples]
        cost = np.zeros_like(y_true, dtype=np.float64)
        cost[minority_samples] = y_true[minority_samples] * (
        (y_pred[minority_samples]) * minority_cost[:, 2] +
        (1 - y_pred[minority_samples]) * minority_cost[:, 1]
        )     
        cost[majority_samples] += (1 - y_true[majority_samples]) * (
        (1 - y_pred[majority_samples]) * majority_cost[:, 3] +
        (y_pred[majority_samples]) * majority_cost[:, 0]
        )
        '''
    ...
    def custom_criterion(self, x, y, cost_mat):
        x1 = x[:, self.u1_index]
        x2 = x[:, self.u2_index]
        ...
        if cost_mat is not None:
            ...
            cost_base = min(self.cost_loss(y, np.zeros(n_samples), cost_mat),
                            self.cost_loss(y, np.ones(n_samples), cost_mat))

            cost = self.cost_loss(y, yp, cost_mat)
            loss_cost = cost / cost_base   
            return  cost
        else:
            return "cost_mat is None" 
...
class PolynomNeuron(Neuron):
    ...
    def fit(self, train_x, train_y, validate_x, validate_y,train_cost_mat,validate_cost_mat, params):
        """
        ...
        self.train_err = self.custom_criterion(train_x, train_y,train_cost_mat)
        self.validate_err = self.custom_criterion(validate_x, validate_y,validate_cost_mat)
...
def fit_layer(fit_layer_data):
    ...
    for neuron in sublayer:
        neuron.fit(fit_layer_data.train_x,
                   ...
                   fit_layer_data.train_cost_mat,
                   fit_layer_data.validate_cost_mat,

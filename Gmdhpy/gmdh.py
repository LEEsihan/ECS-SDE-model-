...
FitData = namedtuple('FitData', ['train_x', 'train_y', 'validate_x', 'validate_y', 'data_x', 'data_y',
                                 ...
                                 'train_cost_mat','validate_cost_mat'])
...
class BaseSONN(object):
    ...
    def _create_layer(self, pool, fit_data):
        ...
        if len(self.layers) > 0:
            ...
            new_fit_data = FitData(train_x, fit_data.train_y, validate_x, fit_data.validate_y, layer_data_x,
                                   fit_data.data_y, fit_data.input_train_x, fit_data.input_validate_x, 
                                   fit_data.input_data_x, fit_data.train_cost_mat,fit_data.validate_cost_mat)
        else:
        ...
        #self._fit_layer(layer, pool, fit_data, fit_params)
    ...
    def fit(self, data_x, data_y, validation_data=None, dataset_split=None,
            verbose=None, cost_mat=None):
        if verbose is not None:
            self.verbose = verbose
        
        if cost_mat is not None:
            data_x, data_y, cost_mat = cost_sampling(data_x, data_y, cost_mat, method='RejectionSampling')

        data_x, data_y = train_preprocessing(data_x, data_y, self.feature_names)

        if validation_data is None:
            input_train_x, train_y, input_validate_x, validate_y,train_cost_mat,validate_cost_mat = split_dataset(
                data_x, data_y,cost_mat, self.param.seq_type)
            input_data_x = data_x
        else:
            input_validate_x, validate_y,validate_cost_mat = train_preprocessing(
                validation_data[0], validation_data[1], validation_data[2],self.feature_names)
            input_train_x = data_x
            train_y = data_y
            train_cost_mat = cost_mat
            input_data_x = np.vstack((input_train_x, input_validate_x))
            data_x = input_data_x
            data_y = np.hstack((train_y, validate_y))
            cost_mat = np.vstack((train_cost_mat, validate_cost_mat))
    ...
    def _predict(self, input_data_x):
        ...
        for n in range(0, len(self.layers)):
            ...
            # Store weights for each neuron in the current layer
            layer_weights = [neuron.w for neuron in prev_layer]
            all_layer_weights.append(layer_weights)

        last_layer = self.layers[-1]
        last_layer_weights = [neuron.w for neuron in last_layer]
        all_layer_weights.append(last_layer_weights)
        ...
        u1 = layer_data_x[:, last_neuron.u1_index]
        u2 = layer_data_x[:, last_neuron.u2_index]
        output_y = last_neuron.transfer(u1, u2, last_neuron.w)

        return output_y, all_layer_weights
    ...
    def _fit_layer(self, layer, pool, fit_data, fit_params):
        ...
        job_args = [FitLayerData(neurons,
                                  fit_data.train_x,
                                    ...
                                    fit_data.validate_x,
                                    fit_data.validate_y, 
                                    fit_data.train_cost_mat,
                                    fit_data.validate_cost_mat,
                                    fit_params
                                    )
...
class Classifier_(BaseSONN):
    ...
    def predict(self, data_x):
        ...
        #return self.le.transform(np.argmax(self.predict_proba(data_x), axis=1))


#aliases
MultilayerGMDH = Classifier_

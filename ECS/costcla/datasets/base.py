...
def load_creditscoring1(cost_mat_parameters=None):
    ...
    raw_data = raw_data.loc[(raw_data['RevolvingUtilizationOfUnsecuredLines'] < 1)]
    ...
    Train = np.hstack((np.array(([Train_y])).T,Train_x))
    test = np.hstack((np.array(([test_y])).T,test_x))
    x_train = Train.astype('float32') / 255.
    x_test = test.astype('float32') / 255.
    # 将数据重塑成N*1的向量
    x_train = x_train.reshape((x_train.shape[0], -1))
    x_test = x_test.reshape((x_test.shape[0], -1))
    # 加入噪声，噪声点成高斯分布，因此均值为0，方差为1
    factor=0.5  # 噪声影响程度
    x_train_noisy = x_train +  factor*np.random.normal(0, 1,x_train.shape) 
    x_test_noisy = x_test + factor*np.random.normal(0,1, x_test.shape) 
    # clip保证数据的范围在0~1之间,不能有负的
    x_train_noisy = np.clip(x_train_noisy, 0,1)
    x_test_noisy = np.clip(x_test_noisy,0,1)
    network = MyAE(x_train = x_train)
    history = network.model.fit(x_train_noisy, x_train, epochs=10,shuffle=True,
                    validation_data=(x_test_noisy, x_test),
                    validation_split=0.2)
    re_test = network.model.predict(x_test_noisy)
    re_train = network.model.predict(x_train_noisy)
    Train = np.vstack((re_test,re_train))
    x = Train[:,1:]
    y = Train[:,0]
    data = pd.DataFrame(x)
    target = y
